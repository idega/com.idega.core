...
/**
 * <p>
 * This Servlet filter sits by default in front of all (Axis) web services and
 * blocks unpriviliged access. <br/>
...
public class WebServiceAuthorizationFilter implements Filter {
	...
	public void doFilter(ServletRequest myRequest, ServletResponse myResponse,
		...
		ServletContext myServletContext = request.getSession()
				.getServletContext();
		// getting the application context
		IWMainApplication mainApplication = IWMainApplication
				.getIWMainApplication(myServletContext);
		boolean doCheck = mainApplication.getIWApplicationContext()
				.getApplicationSettings().getBoolean(
						this.DO_BASIC_AUTHENTICATION, true);
		...
		if (doCheck) {
			...
			try {
				String validIP = mainApplication.getIWApplicationContext()
						.getApplicationSettings()
						.getProperty(this.VALID_IP, "");
	...
	private boolean checkUserPasswordAndRole(HttpServletRequest myRequest,
			String name, String password) {
		// The following code does not work, therefore we are using the own test
		// method
		// if (myRequest.isUserInRole(WEB_SERVICE_USER_ROLE)) {
		// System.out.println("user is in role");
		// }
		ServletContext myServletContext = myRequest.getSession()
				.getServletContext();
		// getting the application context
		IWMainApplication mainApplication = IWMainApplication
				.getIWMainApplication(myServletContext);
	...
	private boolean hasRole(LoginTable loginTable,
			IWMainApplication iwMainApplication) {
		User user = loginTable.getUser();
		List groups = user.getParentGroups();
		Iterator groupIterator = groups.iterator();
		AccessController accessController = iwMainApplication
				.getAccessController();
		while (groupIterator.hasNext()) {
			Group group = (Group) groupIterator.next();
			if (accessController.hasRole(this.WEB_SERVICE_USER_ROLE, group,
					null)) {
				return true;
			}
		}
	...
	private LoginTableHome getLoginTableHome() {
		if (this.loginTableHome == null) {
			try {
				this.loginTableHome = (LoginTableHome) IDOLookup
						.getHome(LoginTable.class);
