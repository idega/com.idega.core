...
public class Gif89Encoder {
	 ...
	 *                     elementType="com.idega.graphics.encoder.gif.Gif89Frame"
	 */
	 ...
	 *              either (1) the aggregate cross-frame RGB color count exceeds
	 *              256, or (2) the Gif89Frame subclass is incompatible with the
	 ...
	 *              cross-frame RGB color count exceeds 256, or (3) this encoder
	 *              object was constructed with an explicit color table.
	 */
	 ...
	 *              IndexGif89Frame <i>per se</i>. But I might add some pedantic
	 *              check later, to justify the generality :)
	 */
	...
	// ----------------------------------------------------------------------------
	/**
	 * A simple driver to test the installation and to demo usage. Put the JAR on
	 * your classpath and run ala <blockquote>java net.jmge.gif.Gif89Encoder
	 * {filename}</blockquote> The filename must be either (1) a JPEG file with
	 * extension 'jpg', for conversion to a static GIF, or (2) a file containing a
	 * list of GIFs and/or JPEGs, one per line, to be combined into an animated
...
class ReverseColorMap {
	 ...
	 * @uml.associationEnd multiplicity="(0 -1)"
	 *                     elementType="com.idega.graphics.encoder.gif.ReverseColorMap$ColorRecord"
	 */
	...
	int getPaletteIndex(int rgb) {
		...
		while ((rec = hTable[itable]) != null && rec.rgb != rgb) {
