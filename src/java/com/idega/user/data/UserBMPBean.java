...
public class UserBMPBean extends AbstractGroupBMPBean implements User, Group, com.idega.core.user.data.User {
	...
	public void initializeAttributes() {
		...
		addAttribute(getColumnNameFirstName(), "First name", true, true, java.lang.String.class, 90);
		addAttribute(getColumnNameMiddleName(), "Middle name", true, true, java.lang.String.class, 90);
		addAttribute(getColumnNameLastName(), "Last name", true, true, java.lang.String.class, 90);
	 ...
	 * 
	 * public String getNameLastFirst(boolean commaSeperated) { String firstName = this.getFirstName(); String middleName = this.getMiddleName(); String
	 * lastName = this.getLastName();
	 * 
	 * if (lastName == null) { lastName = ""; } else { if (commaSeperated) lastName += ","; }
	 * 
	...
	public void setFullName(String fullName) {
		if ((fullName != null) && (fullName.length() > 0)) {
			...
			/*
			 * StringTokenizer token = new StringTokenizer(name); int countWithoutFirstAndLast = token.countTokens() - 2;
			 * 
			 * if (countWithoutFirstAndLast > -2) { setFirstName(((String) token.nextElement())); }
			 * 
			 * if (countWithoutFirstAndLast >= 1) { StringBuffer middleName = new StringBuffer();
			 * 
			 * for (int i = 0; i < countWithoutFirstAndLast; i++) { middleName.append((String) token.nextElement());
			 * 
			 * if (i != (countWithoutFirstAndLast - 1)) middleName.append(" "); }
			 * 
			 * setMiddleName(middleName.toString()); } else { //set middle name == null this.removeFromColumn(this.getColumnNameMiddleName()); }
			 * 
			 * if (countWithoutFirstAndLast >= 0) { setLastName((String) token.nextElement()); } else { //remove last name
	...
	/**
	 * Delete this instance, store timestamp and theid of the user that causes the the erasure
	 * 
	 * @param userId
	 *          id of the user that is responsible for the deletion
	 */
	 ...
	 * 
	 * @param groupList
	 *          a Group of type "UserRepresentative"
	...
	public Integer ejbFindUserForUserRepresentativeGroup(Group userRepGroup) throws FinderException {
		...
		query.appendWhereEqualsQuoted(this.getIDColumnName(), sGroupPK).appendAnd();
	...
	/**
	 * Gets all users that are not marked as deleted and that are members of the specified group
	 * 
	 * @param group
	 * @return Collection
	 * @throws FinderException
	...
	public Collection ejbFindUsersInPrimaryGroup(Group group) throws FinderException {
		IDOQuery query = idoQueryGetSelect();
		query.appendWhere().appendEqualsQuoted(_COLUMNNAME_PRIMARY_GROUP_ID, group.getPrimaryKey().toString()).appendAnd();
		appendIsNotDeleted(query);
		return idoFindIDsBySQL(query.toString());
	}

	/**
	 * Gets all users that are not marked as deleted ordered by first name. This query uses a LoadTracker if necessary.
	 * 
	 * @return Collection
	 * @throws FinderException
	 */
	...
	/*
	 * public boolean equals(Group p0) throws java.rmi.RemoteException { //throw new java.lang.UnsupportedOperationException("Method equals() not yet
	 * implemented."); //return equals((Object)this); return this.getGeneralGroup().equals(p0); }
	 */
	...
	public Integer ejbFindByFirstSixLettersOfPersonalIDAndFirstNameAndLastName(String personalId, String first_name, String last_name) throws FinderException {
		...
		IDOQuery query = idoQueryGetSelect();
		query.appendWhere(getColumnNamePersonalID()).appendLike().appendWithinSingleQuotes(personalId.substring(0, 6) + "%").appendAnd().append(getColumnNameFirstName()).appendEqualSign().appendWithinSingleQuotes(first_name).appendAnd().append(getColumnNameLastName()).appendEqualSign().appendWithinSingleQuotes(last_name).appendAnd();
		appendIsNotDeleted(query);
	 ...
	 * Returns the User that is the instance of the User representing the group userRepGroup
	 * 
	 * @param userRepGroupID
	 *          a primary key of a Group of type "UserRepresentative"
	 * @return Integer the primary key of the User representing the UserGroup
	 * @throws FinderException
	 *           If an error occurs or no user is found for the Group
	 */
	public Integer ejbFindUserForUserGroup(int userRepGroupID) throws FinderException {
		...
		query.appendSelect().append(getIDColumnName()).appendFrom().append(getEntityName()).appendWhereEquals(_COLUMNNAME_USER_GROUP_ID, userRepGroupID).appendAnd();
		appendIsNotDeleted(query);
	 ...
	 * @param condition
	 * @param validUserPks
	 *          if NULL, the function ignores it, else uses it.
	 * @return Collection
	 * @throws FinderException
	 */
	...
	// public Collection ejbFindUsersByConditions(String firstName, String middleName, String lastName, String personalId, String streetName, String
	// groupName, int gender, int statusId, int startAge, int endAge, String[] allowedGroups, String[] allowedUsers, boolean useAnd, boolean
	...
	private String getUserStatusSearchString(int statusId, IWTimestamp currentTime) {
		...
		sql.append("select distinct(ic_user_id) from ic_usergroup_status where status_id = ").append(statusId);
		sql.appendAnd().append("date_from").appendLessThanOrEqualsSign().append(currentTime).appendAnd().appendLeftParenthesis().append("date_to").appendIsNull().appendOr().append("date_to").appendLessThanSign().append(currentTime).appendRightParenthesis();
	...
	private String getUserInAllowedGroupsSearchString(String condition, String[] allowedGroups) {
		...
		if (condition != null) {
			sql.append(" and g.name like '%").append(condition).append("%' ");
		}

		if (allowedGroups != null && allowedGroups.length > 0) {
			sql.append(" and gr.ic_group_id in ( ").append(IDOUtil.getInstance().convertArrayToCommaseparatedString(allowedGroups)).append(" ) ");
		}
	...
	private SelectQuery getUserAddressSelectQuery(String streetName) throws IDOLookupException {
		...
		// sql.append("select ua.ic_user_id from ic_address a,ic_user_address ua where a.ic_address_id=ua.ic_address_id").append(" and a.street_name like
		// '%").append(condition.toUpperCase()).append("%' ");
	...
	public Collection ejbFindUsers(String[] userIDs) throws FinderException {
		IDOQuery query = idoQuery();
		query.appendSelectAllFrom(this).appendWhere().append(getColumnNameUserID()).appendInArray(userIDs).appendAnd();
		appendIsNotDeleted(query);
	...
	public Collection ejbFindUsersInQuery(IDOQuery query) throws FinderException {
		...
		sqlQuery.appendSelectAllFrom(this).appendWhere(getColumnNameUserID()).appendIn(query).appendAnd();
	...
	public Collection ejbFindUsersByCreationTime(IWTimestamp firstCreationTime, IWTimestamp lastCreationTime) throws FinderException, IDOLookupException {
		try {
			...
			// select gr.created,usr.* from IC_USER usr, IC_GROUP gr WHERE usr.IC_USER_ID=gr.IC_GROUP_ID AND created > '18.11.2002 16:31:06' ORDER BY
			// gr.CREATED desc
	...
	public Collection ejbFindByDateOfBirthAndGroupRelationInitiationTimeAndStatus(Date firstBirthDateInPeriode, Date lastBirthDateInPeriode, Group relatedGroup, Timestamp firstInitiationDateInPeriode, Timestamp lastInitiationDateInPeriode, String[] relationStatus) throws IDOLookupException, FinderException {
		// select usr.* from ic_user usr, ic_group_relation gr_rel where usr.date_of_birth >= '23.12.1898' and usr.date_of_birth <= '23.12.1920' and
		// gr_rel.ic_group_id = 3 and gr_rel.related_ic_group_id=usr.ic_user_id and gr_rel.initiation_date >= '11.7.2002 15:17:39' and
		// gr_rel.initiation_date <= '11.7.2002 15:17:40' and gr_rel.group_relation_status in ('ST_ACTIVE')
		try {
			// preparing
			...
			/*
			 * //order by usr.last_name, usr.first_name, usr.middle_name String[] order = new String[3]; order[0] =
			 * variables[0]+"."+thisDef.findFieldByUniqueName(User.FIELD_LAST_NAME).getSQLFieldName(); order[1] =
			 * variables[0]+"."+thisDef.findFieldByUniqueName(User.FIELD_MIDDLE_NAME).getSQLFieldName(); order[2] =
			 * variables[0]+"."+thisDef.findFieldByUniqueName(User.FIELD_MIDDLE_NAME).getSQLFieldName(); query.appendOrderBy(order);
			 */
			// orderby personal_id
	...
	public Collection ejbFindByGroupRelationInitiationTimeAndStatus(Group relatedGroup, Timestamp firstInitiationDateInPeriode, Timestamp lastInitiationDateInPeriode, String[] relationStatus) throws IDOLookupException, FinderException {
		// select usr.* from ic_user usr, ic_group_relation gr_rel where gr_rel.ic_group_id = 3 and gr_rel.related_ic_group_id=usr.ic_user_id and
		// gr_rel.initiation_date >= '11.7.2002 15:17:39' and gr_rel.initiation_date <= '11.7.2002 15:17:40' and gr_rel.group_relation_status in
		// ('ST_ACTIVE')
		try {
			// preparing
	...
	private void appendIsNotDeleted(IDOQuery query) {
		...
		.appendLeftParenthesis().appendEqualsQuoted(getColumnNameDeleted(), GenericEntity.COLUMN_VALUE_FALSE).appendOr().append(getColumnNameDeleted()).append(" IS NULL ").appendRightParenthesis();
	}
	...
	private void appendIsNotDeleted(StringBuffer buffer) {
		buffer.append('(').append(getColumnNameDeleted()).append(" = '").append(GenericEntity.COLUMN_VALUE_FALSE).append("' OR ").append(getColumnNameDeleted()).append(" IS NULL )");
	}
