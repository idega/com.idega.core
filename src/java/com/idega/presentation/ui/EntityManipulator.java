...
public EntityManipulator(GenericEntity[] entity){
	super();
	setEntity(entity);
}
...
public GenericEntity getFirstEntity(){
	return entity[0];
}
...
public PresentationObject getComponent(String columnName,int rowIndex){
	...
	if (getEntity()[rowIndex-1].getRelationShipType(columnName).equals("one-to-very-many")){
		...
		GenericEntity entity = (GenericEntity)getEntity()[rowIndex-1].getColumnValue(columnName);
...
/**
 * Sets a valid range for selection in referential columns as a list of GenericEntity objects i.e. columns that references other tables
 */
...
/**
 * gets the valid range for selection in referential columns as a list of GenericEntity objects i.e. columns that references other tables. Returns null if there is no constraint
 */
