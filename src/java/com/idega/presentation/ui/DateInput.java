...
public class DateInput extends InterfaceObject implements InputHandler {
    ...
    public void addScriptElements(IWContext iwc) {
        if (isShowDay) {
            if (showYear) {
                theYear.setOnChange("iwPopulateDaysWithYear(this,this.form."
                        + theMonth.getName() + ",this.form." + theDay.getName()
                        + ",'" + dayString
                        + "');iwSetValueOfHiddenDateWithAllInputs(this.form."
                        + theYear.getName() + ",this.form."
                        + theMonth.getName() + ",this.form." + theDay.getName()
                        + ",this.form." + theWholeDate.getName() + ");");
                theMonth.setOnChange("iwPopulateDaysWithYear(this.form."
                        + theYear.getName() + ",this,this.form."
                        + theDay.getName() + ",'" + dayString
                        + "');iwSetValueOfHiddenDateWithAllInputs(this.form."
                        + theYear.getName() + ",this.form."
                        + theMonth.getName() + ",this.form." + theDay.getName()
                        + ",this.form." + theWholeDate.getName() + ");");
                theDay
                        .setOnChange("iwSetValueOfHiddenDateWithAllInputs(this.form."
                                + theYear.getName()
                                + ",this.form."
                                + theMonth.getName()
                                + ",this.form."
                                + theDay.getName()
                                + ",this.form."
                                + theWholeDate.getName() + ");");
            } else {
                theMonth.setOnChange("iwPopulateDaysWithMonth('" + selectedYear
                        + "',this,this.form." + theDay.getName() + ",'"
                        + dayString + "');iwSetValueOfHiddenDateWithDay('"
                        + selectedYear + "',this.form." + theMonth.getName()
                        + ",this.form." + theDay.getName() + ",this.form."
                        + theWholeDate.getName() + ");");
                theDay.setOnChange("iwSetValueOfHiddenDateWithDay('"
                        + selectedYear + "',this.form." + theMonth.getName()
                        + ",this.form." + theDay.getName() + ",this.form."
                        + theWholeDate.getName() + ");");
            }
            ...
            if (showYear) {
                theYear.setOnChange("iwSetValueOfHiddenDateWithYear(this.form."
                        + theYear.getName() + ",this.form."
                        + theMonth.getName() + ",this.form."
                        + theWholeDate.getName() + ");");
                theMonth
                        .setOnChange("iwSetValueOfHiddenDateWithYear(this.form."
                                + theYear.getName()
                                + ",this.form."
                                + theMonth.getName()
                                + ",this.form."
                ...
                theMonth.setOnChange("iwSetValueOfHiddenDateWithMonth('"
                        + selectedYear + "',this.form." + theMonth.getName()
                        + ",'01',this.form." + theWholeDate.getName() + ");");
            }
        }
        if (isSetAsNotEmpty) {
            if (showYear) {
        ...
        if (earliestDate != null) {
            if (getParentForm() != null) {
                ...
                buffer.append(
                        "function checkEarliestDate(input, date, warnMsg) {")
                        .append("\n\t");
                ...
                buffer.append("var month = dateString.substring(5,7) - 1;")
                        .append("\n\t\t");
                buffer
                        .append(
        ...
        if (latestDate != null) {
            if (getParentForm() != null) {
                ...
                script.addToFunction("checkSubmit",
                        "if (checkLatestDate (findObj('" + getName() + "'),"
                                + latestDate.getTime() + ", '"
                                + latestDateErrorMessage
                                + "') == false ){\nreturn false;\n}\n");

                StringBuffer buffer = new StringBuffer();
                buffer.append(
                        "function checkLatestDate(input, date, warnMsg) {")
                        .append("\n\t");
                buffer.append("var returnBoolean = true;").append("\n\t");
                buffer.append("var dateString = input.value;").append("\n\t");
                buffer.append("if (dateString.length > 0) {").append("\n\t\t");
    ...
    public void handleKeepStatus(IWContext iwc) {
        if (theDay != null) theDay.handleKeepStatus(iwc);
