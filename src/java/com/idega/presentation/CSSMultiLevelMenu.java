 ...
 * <code>
		...
		level010103.add(new Link("Item two in submenu",CoreConstants.NUMBER_SIGN));
		//one last item
...
public class CSSMultiLevelMenu extends PresentationObjectContainer {
    ...
    /*
     * Constructs and returns a CSSMenu with a Link(menuName,CoreConstants.NUMBER_SIGN) as the label
     */
	...
	public void main(IWContext iwc) throws Exception {
		...
		if(this.addTestData) {
			...
			this.add(level0101);
			//insert items
			level0101.add(new Link("Item one",CoreConstants.HASH));
			level0101.add(new Link("Item two",CoreConstants.HASH));

			//insert item that is a submenu
			CSSMenu level010103 = createCSSMenu("Item three submenu");
			level0101.add(level010103);
			level010103.add(new Link("Item one in submenu",CoreConstants.HASH));
			level010103.add(new Link("Item two in submenu",CoreConstants.HASH));
			//one last item
			level0101.add(new Link("Item four",CoreConstants.HASH));
    ...
    public class CSSMenu extends PresentationObjectContainer{
        ...
        /*
         *  Constructs and adds an Link(CoreConstants.NUMBER_SIGN) object to the menu using the supplied text.
         */
        @Override
