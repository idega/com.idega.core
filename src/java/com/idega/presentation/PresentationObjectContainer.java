...
/**
 * A base class for Containers of PresentationObjects (i.e. that can have
 * children).<br>
 * As of JSF this class is basically obsolete, as all UIComponents are
 * "containers".<br>
 * <br>
...
public class PresentationObjectContainer extends PresentationObject {
	...
	/**
	 * Default constructor. Should only be called by sublasses.
	 */
	...
	public List getChildren() {
		 ...
		 * PresentationObjectList(this); //this.children=new ArrayList(); }
	...
	public void add(Object moduleObject) {
		if (moduleObject instanceof PresentationObject) {
			...
			System.err
					.println("Not instance of PresentationObject and therefore cannot be added to PresentationObjectContainer: "
							+ moduleObject);
		}
	}
			...
			if (children != null) {
				...
				while (iter.hasNext()) {
					...
					if (item instanceof PresentationObjectContainer) {
						...
						List tmp = ((PresentationObjectContainer) item)
	...
	public void _main(IWContext iwc) throws Exception {
		...
		if (mayGoThroughMain()) {
			...
			if (IWMainApplication.useJSF) {
				// Do not go through the children in JSF as that is done through
				// the encode/begin/children methods:
				/*
				 * Iterator iter = getFacetsAndChildren();
				 * while(iter.hasNext()){ UIComponent child =
				 * (UIComponent)iter.next(); if(child instanceof
				 * PresentationObject){ PresentationObject po =
				 * (PresentationObject)child; po._main(iwc); } }
				 */
			} else {
				...
				for (int index = 0; index < numberOfObjects; index++) {
					try {
						...
						log.fine("Exception in PressentationObjectContainer._main() catched: "
	 ...
	 * )newObjToCreate).add((PresentationObject)tempObj.clone()); } // if
	 * (this.theObjects!=null){
	 * //((PresentationObjectContainer)newObjToCreate).setObjects
	 * ((Vector)this.theObjects.clone()); // }
	 */

	/*
	 * public void _print(IWContext iwc) throws Exception { goneThroughMain =
	 * false; super._print(iwc); }
	 */
	/**
	...
	public void print(IWContext iwc) throws Exception {
		...
		// {
		/*
		 * int numberofObjects = numberOfObjects(); for (int index = 0; index <
		 * numberofObjects; index++) { UIComponent tempobj = objectAt(index);
		 * try { if (tempobj != null) { //TL JSF Change: //tempobj._print(iwc);
		 * this.renderChild(iwc,tempobj); flush(); } } catch (Exception ex) {
		 * ExceptionWrapper exep = new ExceptionWrapper(ex, this);
		 * exep._print(iwc); } }
		 */
	...
	public void initVariables(IWContext iwc) throws IOException {
		...
		// This is a legacy fix to make sure the goneThroughMain variable is
		// reset back for
		...
		try {
			try {
				...
				if (isRegion) {
					...
					String regionOwnerInstanceId = instanceId.substring(0,
							instanceId.indexOf("."));
					String index = instanceId.substring(
							instanceId.indexOf(".") + 1, instanceId.length());

					if (index.indexOf(".") == -1) {
						// not a table...don't actually now what kind of object
						// this might be...eiki
						return (((PresentationObjectContainer) getContainedObject(regionOwnerInstanceId))
								.objectAt(Integer.parseInt(index)));
					} else {
						// A region that is a table..
						int xindex = Integer.parseInt(index.substring(0,
								index.indexOf(".")));
						int yindex = Integer.parseInt(index.substring(
								index.indexOf(".") + 1, index.length()));
						try {
							return (((Table) getContainedObject(regionOwnerInstanceId))
									.containerAt(xindex, yindex));
					...
					try {
						...
						while (iter.hasNext()) {
							...
							if (item instanceof PresentationObject
									&& ((PresentationObject) item)
											.getICObjectInstanceID() == instanceIdINT) {
								return item;
							} else if (item instanceof PresentationObjectContainer) {
								UIComponent theReturn = ((PresentationObjectContainer) item)
										.getContainedObject(instanceId);
								if (theReturn != null) {
									return theReturn;
								}
							}
						...
						// must be one of those spiffy new UIComponents and
						// what'not's
						Iterator iter = this.getFacetsAndChildren();
	...
	public UIComponent getContainedLabeledObject(String label) {
		...
		while (iter.hasNext()) {
			UIComponent item = (UIComponent) iter.next();
			if (item instanceof PresentationObjectContainer) {
				String itemLabel = ((PresentationObjectContainer) item)
						.getLabel();
				if (itemLabel != null) {
				...
				UIComponent theReturn = ((PresentationObjectContainer) item)
						.getContainedLabeledObject(label);
				if (theReturn != null) {
					return (theReturn);
				}
			}
	 ...
	 * this.getTreeID(); int numberOfObjects = numberOfObjects(); for(int index
	 * = 0; index < numberOfObjects; index++) { PresentationObject tempobj =
	 ...
	 * (theObjects == null) { this.theObjects = new ArrayList(); } if (modObject
	 * != null) { <<<<<<< PresentationObjectContainer.java
	 * modObject.setParentObject(this); //
	 * modObject.setLocation(this.getLocation());
	...
	/*
	 * public void _setIWContext(IWContext iwc) { setIWContext(iwc); //if
	 * (!isEmpty()) //{ for (int index = 0; index < numberOfObjects(); index++)
	 * { PresentationObject tempobj = (PresentationObject)objectAt(index); if
	 * (tempobj != null) { if (tempobj != this) { tempobj._setIWContext(iwc); }
	 * } } //} }
	 */
	/**
	 * This method is overrided from the PresentationObject superclass here to
	 * call clone(iwc,askForPermission) if askForPermission is true instead of
	 * plain clone() to handle children
	 */
	public Object clonePermissionChecked(IWUserContext iwc,
			boolean askForPermission) {
		if (askForPermission || iwc != null) {
			if (iwc.getApplicationContext().getIWMainApplication()
					.getAccessController().hasViewPermission(this, iwc)) {
	...
	/**
	 * This method can be overridden in subclasses to handle clone of the
	...
	public Object clone(IWUserContext iwc, boolean askForPermission) {
		...
		try {
			...
			/**
			 * TL: Disabled cloning of the list, it shouldn't be necessary:
			 * 
			 * ArrayList alChildren = (ArrayList)myChildren; List clonedChildren
			 * = (List)alChildren.clone(); if(clonedChildren instanceof
			 * PresentationObjectList){ PresentationObjectList pList =
			 * (PresentationObjectList)clonedChildren; pList.setParent(obj); }
	...
	private void cloneJSFChildrenAndFacets(PresentationObject obj,
			IWUserContext iwc, boolean askForPermission) {
		// Cloning the JSF Facets:
		cloneJSFChildren(obj, iwc, askForPermission);
		cloneJSFFacets(obj, iwc, askForPermission);
		// TODO: move the cloning of this to PresentationObject. Now it is
		// inside PresentationObjectContainer

	}

	protected void cloneJSFChildren(PresentationObject obj, IWUserContext iwc,
			boolean askForPermission) {
		// Cloning the JSF children:
		if (this.childrenList != null) {
			// First clone the children List instance itself:
			obj.childrenList = (List) ((PresentationObjectComponentList) this.childrenList)
					.clone();
			((PresentationObjectComponentList) obj.childrenList)
					.setComponent(obj);
			...
			while (iter.hasNext()) {
				...
				if (item instanceof PresentationObject) {
					PresentationObject newObject = (PresentationObject) ((PresentationObject) item)
							.clonePermissionChecked(iwc, askForPermission);
					...
					try {
						BuilderService builderService = getBuilderService(IWMainApplication
								.getDefaultIWApplicationContext());
						UIComponent newUIObject = builderService
								.getCopyOfUIComponentFromIBXML((UIComponent) item);
						// insert the new item
	...
	protected void cloneJSFFacets(PresentationObject obj, IWUserContext iwc,
			boolean askForPermission) {
		// First clone the facet Map:
		if (this.facetMap != null) {
			obj.facetMap = (Map) ((PresentationObjectComponentFacetMap) this.facetMap)
					.clone();
			((PresentationObjectComponentFacetMap) obj.facetMap)
					.setComponent(obj);

			// Iterate over the children to clone each child:
			for (Iterator iter = getFacets().keySet().iterator(); iter
					.hasNext();) {
				String key = (String) iter.next();
				UIComponent component = getFacet(key);
				if (component instanceof PresentationObject) {
					PresentationObject newObject = (PresentationObject) ((PresentationObject) component)
							.clonePermissionChecked(iwc, askForPermission);
					newObject.setParentObject(obj);
					newObject.setLocation(this.getLocation());
	...
	public void restoreState(FacesContext context, Object state) {
		...
		/*
		 * try{ super.restoreState(context, values[0]); }
