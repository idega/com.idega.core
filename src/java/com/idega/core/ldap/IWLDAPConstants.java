...
public interface IWLDAPConstants {
	...
	/**
	 * not the same as uid but a generated 36 character unique identifier, see
	 * GenericEntity
	 */
	...
	/**
	 * User password. The value must be prefixed with LDAP_USER_PASSWORD_PREFIX(
	 * that is the string {md5} ) or similar<br>
	 ...
	 * values of the attribute until a value matching the above syntax is found.
	 * Only if encryptedpassword is an empty string does the user have no
	 * password. DUAs are not required to consider encryption schemes which the
	 * client will not recognize; in most cases, it may be sufficient to
	 * consider only "crypt". Below is an example of a userPassword attribute:
