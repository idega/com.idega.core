...
public class PhoneBMPBean extends com.idega.data.GenericEntity implements com.idega.core.data.Phone
{
	...
	public static int getHomeNumberID()
	{
		...
		try
		{
			PhoneType[] pt =
				(PhoneType[])
					(((com.idega.core.data.PhoneTypeHome) com.idega.data.IDOLookup.getHomeLegacy(PhoneType.class)).createLegacy()).findAllByColumn(
					com.idega.core.data.GenericTypeBMPBean.getColumnNameUniqueName(),
					com.idega.core.data.PhoneTypeBMPBean.UNIQUE_NAME_HOME_PHONE);
			if (pt.length > 0)
			{
				returner = pt[0].getID();
			}
		...
		try
		{
			PhoneType[] pt =
				(PhoneType[])
					(((com.idega.core.data.PhoneTypeHome) com.idega.data.IDOLookup.getHomeLegacy(PhoneType.class)).createLegacy()).findAllByColumn(
					com.idega.core.data.GenericTypeBMPBean.getColumnNameUniqueName(),
					com.idega.core.data.PhoneTypeBMPBean.UNIQUE_NAME_WORK_PHONE);
			if (pt.length > 0)
			{
				returner = pt[0].getID();
			}
		...
		try
		{
			PhoneType[] pt =
				(PhoneType[])
					(((com.idega.core.data.PhoneTypeHome) com.idega.data.IDOLookup.getHomeLegacy(PhoneType.class)).createLegacy()).findAllByColumn(
					com.idega.core.data.GenericTypeBMPBean.getColumnNameUniqueName(),
					com.idega.core.data.PhoneTypeBMPBean.UNIQUE_NAME_FAX_NUMBER);
			if (pt.length > 0)
			{
				returner = pt[0].getID();
			}
		...
		try
		{
			PhoneType[] pt =
				(PhoneType[])
					(((com.idega.core.data.PhoneTypeHome) com.idega.data.IDOLookup.getHomeLegacy(PhoneType.class)).createLegacy()).findAllByColumn(
					com.idega.core.data.GenericTypeBMPBean.getColumnNameUniqueName(),
					com.idega.core.data.PhoneTypeBMPBean.UNIQUE_NAME_MOBILE_PHONE);
			if (pt.length > 0)
			{
				returner = pt[0].getID();
			}
