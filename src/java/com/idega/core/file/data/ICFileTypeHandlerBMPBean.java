...
public class ICFileTypeHandlerBMPBean extends com.idega.data.CacheableEntityBMPBean implements com.idega.core.file.data.ICFileTypeHandler {
	...
	public void insertStartData() {
		try {
			...
			handler.store();
			handler = ftHome.create();
			handler.setNameAndHandlerClass(IC_FILE_TYPE_HANDLER_APPLICATION, RefactorClassRegistry.forName("com.idega.block.media.business.ApplicationTypeHandler"));
			handler.store();
			handler = ftHome.create();
			...
			handler.store();
			handler = ftHome.create();
			handler.setNameAndHandlerClass(IC_FILE_TYPE_HANDLER_DOCUMENT, RefactorClassRegistry.forName("com.idega.block.media.business.DocumentTypeHandler"));
			handler.store();
			handler = ftHome.create();
			...
			handler.store();
			handler = ftHome.create();
			handler.setNameAndHandlerClass(IC_FILE_TYPE_HANDLER_VECTOR_GRAPHICS, RefactorClassRegistry.forName("com.idega.block.media.business.VectorTypeHandler"));
			handler.store();
			handler = ftHome.create();
	...
	public Object ejbFindByName(String name) throws FinderException {
		...
		query.addCriteria(new MatchCriteria(table.getColumn(getColumnNameHandlerName()), MatchCriteria.EQUALS, name));
