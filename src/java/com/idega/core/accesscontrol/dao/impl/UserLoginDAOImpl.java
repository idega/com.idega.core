...
public class UserLoginDAOImpl extends GenericDaoImpl implements UserLoginDAO {
	...
	public UserLogin findLoginForUser(User user) {
		...
		List<UserLogin> logins = getResultList("login.findDefaultLoginForUser", UserLogin.class, param);
	...
	public UserLogin findLoginByUsername(String username) {
		...
		List<UserLogin> logins = getResultList(UserLogin.QUERY_FIND_BY_LOGIN, UserLogin.class, param);
	...
	public LoginRecord getLastRecordByUser(User user) {
		Param param = new Param("user", user);
		List<LoginRecord> records = getResultList("loginRecord.findLastByUser", LoginRecord.class, param);
		return ListUtil.isEmpty(records) ? null : records.get(0);
	}
