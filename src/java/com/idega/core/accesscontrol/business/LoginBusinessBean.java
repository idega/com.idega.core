...
/*		com.idega.core.user.data.UserHome uHome = (com.idega.core.user.data.UserHome) com.idega.data.IDOLookup.getHome(User.class);
		...
		com.idega.user.business.UserBusiness userbusiness =
			(com.idega.user.business.UserBusiness) com.idega.business.IBOLookup.getServiceInstance(
				iwc,
		...
		if (userGroupId != -1)
		{
			LoginBusinessBean.setUserRepresentativeGroup(
				iwc,
				(
		...
		if (user.getPrimaryGroupID() != -1)
		{
			GenericGroup primaryGroup =
				((com.idega.core.data.GenericGroupHome) com.idega.data.IDOLookup.getHomeLegacy(GenericGroup.class)).findByPrimaryKeyLegacy(
