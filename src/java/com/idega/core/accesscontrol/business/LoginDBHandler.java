...
public class LoginDBHandler
{
	...
	protected static int createLoginInfo(
		boolean update,
		int loginTableID,
		Boolean accountEnabled,
		idegaTimestamp modified,
		int daysOfVality,
	...
	public static LoginTable getUserLoginByUserName(String userName)
	{
		try
		{
			LoginTable[] login_table =
				(LoginTable[]) (LoginTableBMPBean.getStaticInstance()).findAllByColumn(
					LoginTableBMPBean.getUserLoginColumnName(),
					userName);
	...
	public static boolean isLoginInUse(String login)
	{
		try
		{
			return (
				0
					< com.idega.core.accesscontrol.data.LoginTableBMPBean.getStaticInstance().getNumberOfRecords(
						com.idega.core.accesscontrol.data.LoginTableBMPBean.getUserLoginColumnName(),
						login));
		}
		catch (Exception ex)
	...
	{
		com.idega.core.user.data.UserHome uHome =
			(com.idega.core.user.data.UserHome) com.idega.data.IDOLookup.getHome(User.class);
