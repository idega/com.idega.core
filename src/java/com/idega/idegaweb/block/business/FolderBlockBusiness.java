...
public class FolderBlockBusiness {
  ...
  public ICInformationFolder getInstanceWorkeFolder(int icObjectInstanceId, int icObjectId, int localeId, boolean autocreate){
    ...
    try {
      List l =EntityFinder.findRelated(new ICObjectInstance(icObjectInstanceId),ICInformationFolder.getStaticInstance(ICInformationFolder.class));
      if(l != null && l.size() > 0){
    ...
    if(parentFolder != null){
      try {
        List l = EntityFinder.findAllByColumn(ICInformationFolder.getStaticInstance(ICInformationFolder.class),ICInformationFolder.getColumnParentFolderId(),parentFolder.getID(),ICInformationFolder.getColumnLocaleId(),localeId);
        if(l != null && l.size() > 0){
  ...
  public List getInstanceCategories(int icObjectInstanceId){
     try {
      List l =EntityFinder.findRelated(new ICObjectInstance(icObjectInstanceId),ICInformationCategory.getStaticInstance(ICInformationCategory.class));
      return l;
    }
    catch (SQLException ex) {
      ex.printStackTrace();
  ...
  public ICInformationCategory createICInformationCategory(String name, String description, String type, int ICObjectId, int ownerFolder) throws SQLException{
    ICInformationCategory cat = new ICInformationCategory();
